   ...
   // public $verification_with;
    ...
    public function rules()
    {
        return [
            ...
            [['device_token','device_token_voip_ios','description','bio','country_code','phone','dob','country','city','paypal_id','socket_id','location','latitude','longitude','occupation','qualification','drinking_habit','religion','color','height','state','website'], 'string'],
            [['sex','available_coin','is_email_verified','is_phone_verified','account_created_with','is_biometric_login','is_push_notification_allow','like_push_notification_status','comment_push_notification_status','is_chat_user_online','chat_last_time_online','smoke_id','marital_status','country_id','state_id','city_id','work_experience_month','work_experience_year','profile_category_type','profile_visibility'], 'integer'],
            //[['email'], 'email'],
    ...
    {
        return ['isFollowing','isFollower','totalFollowing','totalFollower','totalPost','totalActivePost','totalWinnerPost','totalReel','totalClub','totalMention','userLiveDetail','giftSummary','follower','following','interest','userSetting','language'];
    }
    ...
    public function getCoverImageUrl()
    {
        if($this->cover_image){
            $modelFileUpload = new FileUpload();
            return $modelFileUpload->getFileUrl($modelFileUpload::TYPE_USER,$this->cover_image);
        }else{
            return null;
        }
    ...
    {
      return  $this->find()->select(['id','name','username','email','bio','description','image','cover_image','user.is_verified','user.country_code','user.phone','user.country','user.city','user.sex','user.dob','user.paypal_id','user.available_balance','user.available_coin','user.is_biometric_login','is_push_notification_allow','account_created_with','auth_key','is_login_first_time'])->where(['id' => $id])->one();
    }
    ...
    public function getFullProfile($id)
    {
        ...
        ->with(['userSetting'=> function ($query) {
